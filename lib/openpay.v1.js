(function() {
    ...
    this.OpenPay = function() {
    	...
    	_openpay.setCustomerId = function(t){
    		_openpay.customerId = t;
    	},
    	...
    	_openpay.send = function(_endpoint, _data, _success, _failure) {
        	...
        	if (_openpay.customerId !== '' && !(/^[a-z0-9]+$/i.test(_openpay.customerId)) ) {
        		handleError('Invalid OpenPay Customer ID');
        		return;
        	}
    ...
    this.OpenPay.card = function(_parent) {
    	...
    	_card.charge = function(_params, _success, _failure) {
    		return INSTANCE.validate(_params, "tarjeta"), INSTANCE.formatData(_params, _card.whitelistedAttrs), INSTANCE.send('charges', _params, _success, _failure);
    	},
    	_card.create = function(_params, _success, _failure) {
    		var _customerEndpoint = 'cards';
    		if (typeof INSTANCE.customerId !== 'undefined' && INSTANCE.customerId !== ''){
    			_customerEndpoint = 'customers/' + INSTANCE.customerId + '/cards';
    		}
